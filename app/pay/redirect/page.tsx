...
import { devCompletePayment } from "../../p/actions";

/**
 * Secure payment interstitial — /pay/redirect.
 * Real buyers go straight to the provider's hosted checkout, so this screen
 * serves two cases:
 *   1. DEV MODE (?ref=...&dev=1): stands in for the hosted checkout — after a
 *      short pause it runs the real success path (settle, notify, payout leg)
 *      via devCompletePayment, then lands on the callback.
 *   2. The sample/prototype checkout: simulates a success redirect.
 */
function RedirectContent() {
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      if (dev && ref && token) {
        await devCompletePayment({ reference: ref, token });
        router.replace(`/pay/callback?reference=${encodeURIComponent(ref)}`);
        return;
      }
      router.replace("/pay/callback?ref=PP-7F3A9C2E&status=success");
    }, 1800);
    return () => clearTimeout(t);
  }, [router, dev, ref, token]);
